...
class SessionTest < Minitest::Test
  ...
  def test_session_can_be_started
    ...
    assert session = Authentication::Session.start(controller)
    ...
    assert_equal session.token_hash, Authentication::Session.hash_token(controller.cookies[:user_session])
  end
  ...
  def test_sessions_can_be_retreived_from_controller
    ...
    assert_equal session, Authentication::Session.get_session(controller)
  end
  ...
  def test_using_a_session_from_another_browser_fails
    ...
    assert new_session = Authentication::Session.get_session(other_controller)
    # Test that the security check raises an error
    assert_raises Authentication::Session::BrowserMismatch do
  ...
  def test_the_raw_token_is_available_when_looked_up
    ...
    original_session = Authentication::Session.start(controller)
    ...
    session = Authentication::Session.get_session(controller)
  ...
  def test_using_inactive_sessions_fails
    ...
    assert_raises Authentication::Session::InactiveSession do
  ...
  def test_using_persistent_token_after_expiry_fails
    ...
    # Test that the security check raises an appropriate error
    assert_raises Authentication::Session::ExpiredSession do
      session.check_security!
    end
  end
  ...
  def test_using_inactive_session_fails
    ...
    assert session.update(:last_activity_at => (Authentication.config.session_inactivity_timeout + 1.minute).ago)
    # Test that the security check raises an appropriate error
    assert_raises Authentication::Session::InactiveSession do
      session.check_security!
    end
  ...
  def test_using_session_from_another_host_fails
    ...
    assert session = Authentication::Session.get_session(controller)
    assert_raises Authentication::Session::HostMismatch do
      session.check_security!
    end
  end
  ...
  def test_cookie_properties_are_set
    ...
    assert_equal session.token_hash, Authentication::Session.hash_token(controller.cookies.raw[:user_session][:value])
  ...
  def test_sessions_can_have_arbitary_data_stored_within
    ...
    new_session = Authentication::Session.get_session(controller)
  ...
  def test_sessions_can_be_associated_with_users
    ...
    controller = FakeController.new
    # Make a session
    assert session = Authentication::Session.start(controller, :user => user)
  ...
  def test_sessions_can_invalidate_all_other_sessions_for_a_user
    ...
    assert session1 = Authentication::Session.start(FakeController.new(:browser_id => 'b1'), :user => user)
    assert session2 = Authentication::Session.start(FakeController.new(:browser_id => 'b2'), :user => user)
    assert session3 = Authentication::Session.start(FakeController.new(:browser_id => 'b3'), :user => user)
  ...
  def test_user_impersonation
    ...
    assert session = Authentication::Session.start(controller, :user => user1)
    # Impersonate a user and test that a new session is returned
    assert new_session = session.impersonate!(user2)
    assert_equal Authentication::Session, new_session.class
    ...
    assert_equal new_session.token_hash, Authentication::Session.hash_token(controller.cookies[:user_session])
