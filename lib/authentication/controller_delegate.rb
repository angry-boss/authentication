...
module Authentication
  class ControllerDelegate
    ...
    def set_browser_id
      until cookies[Authentication.config.browser_id_cookie_name]
        proposed_browser_id = SecureRandom.uuid
        unless Authentication::Session.where(:browser_id => proposed_browser_id).exists?
          cookies[Authentication.config.browser_id_cookie_name] = {
          ...
          Authentication.config.events.dispatch(:set_browser_id, proposed_browser_id)
        end
      end
    end
    ...
    def create_auth_session(user)
      if user
        @auth_session = Authentication::Session.start(@controller, :user => user)
      else
    ...
    def auth_session
      @auth_session ||= Authentication::Session.get_session(@controller)
