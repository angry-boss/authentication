...
module Authentication
  class Session < ActiveRecord::Base
    ...
    # Set table name
    self.table_name = "authentication_sessions"

    # Relationships
    parent_options = {:class_name => "Authentication::Session"}
    ...
    def touch!
      ...
      self.requests += 1
      self.save!
      Authentication.config.events.dispatch(:session_touched, self)
      true
    end
    ...
    def set_cookie!
      ...
      Authentication.config.events.dispatch(:session_cookie_updated, self)
      true
    end
    ...
    def check_security!
      if controller
        if cookies[:browser_id] != self.browser_id
          invalidate!
          Authentication.config.events.dispatch(:browser_id_mismatch_error, self)
          raise BrowserMismatch, "Browser ID mismatch"
        end

        unless self.active?
          invalidate!
          Authentication.config.events.dispatch(:invalid_session_error, self)
          raise InactiveSession, "Session is no longer active"
        end

        if self.expired?
          invalidate!
          Authentication.config.events.dispatch(:expired_session_error, self)
          raise ExpiredSession, "Persistent session has expired"
        end

        if self.inactive?
          invalidate!
          Authentication.config.events.dispatch(:inactive_session_error, self)
          raise InactiveSession, "Non-persistent session has expired"
        end

        if self.host && self.host != controller.request.host
          invalidate!
          Authentication.config.events.dispatch(:host_mismatch_error, self)
    ...
    def inactive?
      ...
      self.last_activity_at < Authentication.config.session_inactivity_timeout.ago
    end

    # Allow this session to persist rather than expiring at the end of the
    # current browser session
    def persist!
      self.expires_at = Authentication.config.persistent_session_length.from_now
      self.save!
      set_cookie!
    end
    ...
    def invalidate!
      ...
      Authentication.config.events.dispatch(:session_invalidated, self)
      true
    end
    ...
    def see_password!
      ...
      self.save!
      Authentication.config.events.dispatch(:seen_password, self)
      true
    end
    ...
    def self.start(controller, params = {})
      ...
      session.save!
      Authentication.config.events.dispatch(:start_session, session)
      session
    end

    # Cleanup any old sessions.
    def self.cleanup
      Authentication.config.events.dispatch(:before_cleanup)
      # Invalidate transient sessions that haven't been used
      self.active.where("expires_at IS NULL AND last_activity_at < ?", Authentication.config.session_inactivity_timeout.ago).each(&:invalidate!)
      ...
      Authentication.config.events.dispatch(:after_cleanup)
      true
    end
